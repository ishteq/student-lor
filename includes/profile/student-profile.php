<div class="user-profile pull-right">
         <img class="avatar user-thumb" src="../assets/images/avatar.jpg" alt="avatar">
        <h4 class="user-name dropdown-toggle" data-toggle="dropdown"><?php include 'student-logged.php' ?> <i class="fa fa-angle-down"></i></h4>
         <div class="dropdown-menu">
            <!-- <a class="dropdown-item" href="my-profile.php">View Profile</a>
            <a class="dropdown-item" href="change-password-employee.php">Password</a> -->
            <a class="dropdown-item" href="../includes/profile/student-logout.php">Log Out</a>
     </div>
</div>